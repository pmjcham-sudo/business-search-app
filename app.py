...
def search_nps_workplace(place_name):
    ...
    try:
        ...
        st.warning(f"국민연금 API 조회 중 오류: {e}")
        return []
...
def match_nps_employee_count(place_name, road_address, jibun_address):
    ...
    for item in items:
        nps_name = get_value_from_item(
            item,
            ["wkplNm", "wkpl_nm", "사업장명", "bzowrNm", "wkpl_nm"]
        )
        ...
        if kakao_addr_front and nps_addr_front:
            kakao_parts = kakao_addr_front.split()
            nps_parts = nps_addr_front.split()

            if kakao_addr_front == nps_addr_front:
                score += 40
            elif len(kakao_parts) >= 3 and len(nps_parts) >= 3 and kakao_parts[:3] == nps_parts[:3]:
                score += 25
